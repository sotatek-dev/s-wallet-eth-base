...
sota_common_1.EnvConfigRegistry.onNetworkChanged(function (network) {
    ...
    if (network === sota_common_1.NetworkType.TestNet) {
        var provider = new Web3.providers.HttpProvider("https://rinkeby.infura.io/v3/" + infuraProjectId);
        web3.setProvider(provider);
        infuraWeb3.setProvider(provider);
        return;
    }
...
sota_common_1.CurrencyRegistry.onCurrencyConfigSet(function (currency, config) {
    if (currency.symbol !== sota_common_1.BlockchainPlatform.Ethereum && currency.platform !== sota_common_1.BlockchainPlatform.Polygon) {
        return;
    }
