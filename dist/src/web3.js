...
sota_common_1.EnvConfigRegistry.onNetworkChanged(function (network) {
    ...
    if (network === sota_common_1.NetworkType.MainNet) {
        var provider = new Web3.providers.HttpProvider("https://mainnet.infura.io/v3/".concat(infuraProjectId));
        web3.setProvider(provider);
        infuraWeb3.setProvider(provider);
        return;
    }
    if (network === sota_common_1.NetworkType.TestNet) {
        var provider = new Web3.providers.HttpProvider("https://rinkeby.infura.io/v3/".concat(infuraProjectId));
        web3.setProvider(provider);
        infuraWeb3.setProvider(provider);
        return;
    }
...
sota_common_1.CurrencyRegistry.onCurrencyConfigSet(function (currency, config) {
    ...
    logger.info("web3::onCurrencyConfigSet currency=".concat(currency.symbol, " config=").concat(JSON.stringify(config)));
